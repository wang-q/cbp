...
# Initialize autotools build system
# First run autoreconf to generate all necessary files
...
CFLAGS="-I${CBP_INCLUDE} -O -w" \
CXXFLAGS="-I${CBP_INCLUDE} -O -w -std=c++14" \
LDFLAGS="-L${CBP_LIB}" \
...
# Patch str_comp compatibility issue in multiple files
for file in "src2/getSuperReadInsertCountsFromReadPlacementFileTwoPasses.cc" "src2/getSuperReadInsertCountsFromReadPlacementFile.cc"; do
    if [ -f "$file" ]; then
        echo "Patching str_comp compatibility issue in $file..."
        
        # Create backup
        cp "$file" "$file.bak"
        
        # Replace the struct definition using awk for more precise matching
        awk '
        /^struct str_comp \{/ {
            # Print the new struct definition
            print "struct str_comp {"
            print "  bool operator()(const char *s1, const char *s2) const {"
            print "    return strcmp(s1, s2) < 0;"
            print "  }"
            print "  typedef void is_transparent;"
            print "};"
            # Skip until we find the closing brace
            while (getline > 0 && !/^\};?$/) {
                # Skip lines until we find the end of the struct
            }
            next
        }
        { print }
        ...
        echo "Successfully patched str_comp compatibility issue in $file"
    fi
done
...
# ldd "${TEMP_DIR}/collect/bin/create_k_unitigs_large_k"
